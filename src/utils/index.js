...
export const lockFunds = async (swapInfo, swapResponse) => {
  ...
  console.log("rbtc locking with ", preimageHashbuffer, swapResponse.claimAddress, timeout, "to contract ", swapResponse.address);
  // , chainId: 33
  rbtcswapcontract.methods.lock(preimageHashbuffer, swapResponse.claimAddress, timeout).send({from: web3.eth.accounts.currentProvider.selectedAddress, value: amount}, function(error, transactionHash){
